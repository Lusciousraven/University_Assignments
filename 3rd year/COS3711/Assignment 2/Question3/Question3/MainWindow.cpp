...
void MainWindow::setupUi()
{
    ...
    formLayout->addWidget(m_readButton);
    ...
    connect(m_readButton, &QPushButton::clicked, this, &MainWindow::readRegistrationList);
...
void MainWindow::readRegistrationList()
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open Registration List"), "", tr("XML Files (*.xml)"));
    if (fileName.isEmpty())
        return;

    RegistrationListReader reader;
    QList<Registration*> registrations = reader.readFile(fileName);
    if (registrations.isEmpty())
    {
        QMessageBox::warning(this, tr("Error"), tr("Failed to read the registration list or the list is empty."));
        return;
    }
    ...
    QMessageBox::information(this, tr("Success"), tr("Registrations have been successfully read and added."));
}
